...
DC=0;DI=0;DP0=0;
diag_AR=diag(A)+diag(R);  
I=find(diag_AR>0); K=length(I); % Identify exemplars
[diag_s,ind]=sort(diag_AR,'descend');
NN=length(diag_AR); 
...
while K<NN/2   
    ...
    for ii=1:K
        for jj=1:ii
            DC=DC+S(ind(ii),ind(jj));  % between-class (squared) distance
        end
    end
    DC=DC-S(1,1)*K;
    nDC=sum(1:K-1);
    ...
    for i=1:NN
        DI=DI+S(i,idx(i));    % within-class (squared) distance
    end
    DI=DI/NN;
    ...
    if diff<0            % termination conditions
        I=I(1:length(I)-1);
        K=length(I);
        idx=idx0;
        break;
